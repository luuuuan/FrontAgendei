...
export class ClientesComponent implements OnInit {
  clientes: Usuario[] = []; clientesFiltrados: Usuario[] = []; clienteSelecionado: Usuario | null = null;
  carregando = false; erro = ''; modalAberto = false; salvando = false; termoBusca = '';
  form: FormGroup;

  constructor(private fb: FormBuilder, private usuarioService: UsuarioService, private toast: ToastService) {
    this.form = this.fb.group({
      nome: ['', Validators.required], email: ['', [Validators.required, Validators.email]],
      cpf: ['', Validators.required], telefone: ['', [Validators.required, Validators.minLength(10)]],
      senha: ['', [Validators.required, Validators.minLength(8)]],
      dataNascimento: [''], enderecoId: [1], tipoUsuario: ['CLIENTE', Validators.required]
    });
  }
  ...
  filtrar() {
    ...
    this.clientesFiltrados = t ? this.clientes.filter(c => c.nome?.toLowerCase().includes(t) || c.email?.toLowerCase().includes(t)) : this.clientes;
  }

  selecionarCliente(c: Usuario) { this.clienteSelecionado = c; }
  iniciais(nome: string) { if (!nome) return '?'; const p = nome.trim().split(' '); return p.length === 1 ? p[0][0].toUpperCase() : (p[0][0] + p[p.length-1][0]).toUpperCase(); }
