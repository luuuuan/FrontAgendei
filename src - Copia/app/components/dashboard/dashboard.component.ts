...
export class DashboardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.usuarioService.listar().subscribe({ next: (l) => { this.totalClientes = l.length; this.carregandoClientes = false; }, error: () => this.carregandoClientes = false });
    this.carregandoServicos = true;
    this.servicoService.listar().subscribe({ next: (l) => { this.totalServicos = l.length; this.carregandoServicos = false; }, error: () => this.carregandoServicos = false });
    this.carregandoProfissionais = true;
    this.profissionalService.listar().subscribe({ next: (l) => { this.totalProfissionais = l.length; this.carregandoProfissionais = false; }, error: () => this.carregandoProfissionais = false });
    this.carregandoAgendamentos = true;
    const hoje = new Date().toISOString().split('T')[0];
    this.agendamentoService.buscarPorData(hoje).subscribe({ next: (l) => { this.agendamentosHoje = l; this.carregandoAgendamentos = false; }, error: () => this.carregandoAgendamentos = false });
  }
