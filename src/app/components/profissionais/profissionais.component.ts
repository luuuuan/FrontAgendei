...
export class ProfissionaisComponent implements OnInit {
  ...
  carregarProfissionais(prestadorId?: number) {
    console.log('prestadorId:', prestadorId); // ← adiciona isso
  ...
  buscarUsuario() {
    const raw = this.buscaUsuario.trim();
    const valor = raw.includes('@') ? raw : raw.replace(/\D/g, '');
  ...
  vincular() {
    ...
    this.profissionalService.vincular(
      ...
      next: () => {
        this.toast.sucesso(`...`);
        this.fecharModal();
        this.carregarProfissionais(sessao.prestadorId); // ← passa aqui também
        this.salvando = false;
      },
      error: (err: any) => {
